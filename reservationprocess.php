...
$lastnameascii_code = ord(substr($Firstname, 0, 3));
$random_number = mt_rand(10000, 99999);
$CustomerID = $Lastname . $lastnameascii_code . $random_number;

$record = mysqli_query($con, "SELECT * FROM customer WHERE CustomerID='$CustomerID'");

if (mysqli_num_rows($record) == 0) {
    $sqlInsert2 = mysqli_query($con, "INSERT INTO customer (CustomerID, First_Name, Last_Name, Telephone_No, Address, Email) 
        VALUES ('$CustomerID', '$Firstname', '$Lastname', '$Telephone_No', '$Address', '$Email')");
} else {
